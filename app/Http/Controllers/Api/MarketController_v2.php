...
class MarketController_v2 extends Controller
{
    private $token;
    private $port;
    private $hostName;
    ...
    public function getExchangeRates(Request $request) {
        ...
        try {
            $marketResponse = [];
            $coinInfo = $this->coinInfo($request->coin);
            foreach ($request->currencies as $key => $value) {
                    # code...
                    $resp = $this->getDaTa($request->coin,$coinInfo, $value);
                    if ($resp['status']) {
                        $marketResponse[$value] = $resp['data'];
                    }
    ...
    public function coinInfo($coins)
    {
        $marketResponse = [];
        ...
        foreach ($coins as $key => $value) {
            ...
            $marketData = $bitgo->getMarketPriceData();
            $marketResponse[$value] = $marketData;
       }

       return $marketResponse;
    }

    public function getDaTa($coins, $coinInfo, $curency)
    {
        try {
            ...
            foreach ($coins as $key => $value) {
                ...
                $currencies = $coinInfo[$value]['latest']['currencies'];

                $usdRate = $coinInfo[$value]['latest']['currencies']['USD'];
                ...
                if (array_key_exists($curency, $currencies )) {
                    ...
                    $marketResponse[$value] = $coinInfo[$value]['latest']['currencies'][$curency];
                    ...
                    $rateWithUSD = $coinInfo[$value]['latest']['currencies']['USD'];
